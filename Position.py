...
def loadArray(arr_route, arr_measurement_points, arr_turning_points, grid_coordinates, ser, ser1):
    ...
    # for cell in send_point:
    #     xy = map(int, cell.split("/"))
    #     if xy == grid_coordinates:
    #         ser.write(grid_coordinates[0].to_bytes(), grid_coordinates[1].to_bytes())
...
def drive(distance, degrees, blocked,ser):
    count = 0
    count2 = 0 
    median = []
    minimum = []
...
def getGridCoords(filename, ser, ser1):
    ...
    loadArray(arr_route, arr_measurement_points, arr_turning_points, grid_coordinates, ser, ser1)
...
def main():
    ...
    while lidar.open():
        ...
        direction_measured = ser.readline().decode('utf-8').rstrip()
    
        if(last_dir != direction_measured and direction_measured != ""):
            direction = direction_measured
            print(direction)
        ...
        blocked = drive(distance, degrees, blocked, ser)
